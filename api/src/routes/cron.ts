...
// Job: unlock coins for claims where coin_locked_at < now - 48h
//      Check: still subscribed (YouTube API) → CREDITED or CLAWED_BACK

const COIN_LOCK_DURATION = 48 * 3600  // 48 hours in seconds
...
export async function cronHandler(c: Context<{ Bindings: Env; Variables: HonoVariables }>) {
  ...
  const unlockBefore = now - COIN_LOCK_DURATION
  ...
  const ready = await c.env.DB.prepare(`
    ...
    WHERE tc.coin_status = 'LOCKED'
      ...
      AND tc.coin_locked_at < ?
    LIMIT 50
  `).bind(unlockBefore).all<{
    id: string; claimer_id: string; task_id: string;
    youtube_channel_id: string; coin_amount: number;
    channel_id: string; coin_locked_at: number;
  }>()
  ...
  for (const claim of ready.results) {
    ...
    if (stillSubbed) {
      ...
      await c.env.DB.batch([
        ...
        `).bind(now, claim.id),
        c.env.DB.prepare(`
          UPDATE wallets SET coin_pending = coin_pending - ?, coin_balance = coin_balance + ? WHERE user_id = ?
        ...
        c.env.DB.prepare(`
          ...
          VALUES (?,?,?,?,?,?,?)
        `).bind(crypto.randomUUID(), claim.claimer_id, 'EARN', claim.coin_amount, 'COIN', claim.id, 'Coins unlocked after 48h verify'),
      ])
      credited++
    } else {
      // Claw back
      await c.env.DB.batch([
        ...
        `).bind(claim.coin_amount, claim.claimer_id),
        c.env.DB.prepare(`
          INSERT INTO wallet_txns (id, user_id, type, amount, currency, ref_id, note)
          VALUES (?,?,?,?,?,?,?)
        `).bind(crypto.randomUUID(), claim.claimer_id, 'CLAW_BACK', claim.coin_amount, 'COIN', claim.id, 'Unsub detected — coins clawed back'),
      ])
      clawedBack++
    }
  }
...
async function checkStillSubscribed(
  ...
  try {
    ...
    const res = await fetch(
      `https://www.googleapis.com/youtube/v3/channels?part=statistics&id=${buyerChannelId}&key=***`
    )
